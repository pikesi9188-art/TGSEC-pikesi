...
```bash
python3 炼蛊房/binary_diff_probe.py drive --old a --new b --case <案>
```

L2=哈希不同且能点出名串差。证据进案卷 `测绘/`。

补丁反推 CVE / N-day → `patch-diff-exploit`。已知 CVE 打站 → `1day-nuclei-kit` + `nday_route.py`。本卡只搬符号、不写利用。
