---
name: 太白云生·寻傀
description: "TG Bot + Mini App 侦察：Bot 有效性验证、getMe/getWebhookInfo 情报收集、TMA webApp 链接提取、Bot Token 来源溯源。触发：TG bot token/TMA 侦察/bot 验活/webhook 信息收集。"
version: 1.0.0
metadata:
...
本库无此外门 `scripts/enumerate_tma_bots.py`。批量探 bot 用 `getMe` / `getChat` + 本卡字段（`has_mini_app` / `webapp_url`），或 Skill `太白云生·寻傀` 手工清单。Run:

```bash
# 有 user session 时按本卡字段逐个 getChat；不要找死脚本 enumerate_tma_bots.py
...
2. 按本卡字段逐个核验 → JSONL（本库无 `enumerate_tma_bots.py`）
...
- 手法：`传承/乐土·人情.md`
- 工具：`python3 炼蛊房/tg_cmd_enum.py --help`
