...
| 已装技能 | `杀招/{redteam-opsec,xiaolanben-equity,quake}/` | 本库卡 |
...
3. **a_scan.py** — DesRedTeam 提取物，**不在本仓库**。本库广谱侦察用 `python3 tools/pentest-swarm/bin/psa_scan.py`（须已授权 scope）。
...
# 回滚只清 DesRedTeam 提取物，不要删本库 .cursor/skills 专卡
```
...
### FOFA ICP 备案查询（DesRedTeam 特有增量能力）

```bash
# ICP 备案查询（空间眼 FOFA 没有此能力）
python3 ./data/desredteam_assets/recon/fofa.py \
 --query 'icp="沪ICP备XXXX号"'
...
```bash
# 查看可用工具列表
ls ./data/desredteam_assets/tools/*.yaml | xargs -I{} python3 -c "
import yaml, sys
with open('{}') as f:
 data = yaml.safe_load(f)
print(data['name'], '-', data.get('short_description',''))
" 2>/dev/null | sort

# 查询特定工具参数（如 ffuf）
python3 -c "
import yaml
with open('./data/desredteam_assets/tools/ffuf.yaml') as f:
 data = yaml.safe_load(f)
print('Command:', data['command'])
print('Args:', data.get('args',''))
for p in data.get('parameters', []):
 req = '(required)' if p.get('required') else '(optional)'
 print(f' -{p[\"flag\"]} {p[\"type\"]} {req}: {p[\"name\"]}')
" 2>/dev/null
```
...
# 大批量文件传输（打包后单次 scp）
tar czf /tmp/bundle.tar.gz /path/to/files/
scp /tmp/bundle.tar.gz root@<授权主机>:/tmp/
# 连接间隔 ≥ 30s
sleep 30
ssh root@<授权主机> "tar xzf /tmp/bundle.tar.gz -C /target/"
```

### 小蓝本股权溯源（供应链）

```bash
# xiaolanben-equity 技能触发（从公司递归找子公司域名）
# 注意 err002 签名需预热
python3 tools/space-search/bin/space_search.py \
 --query 'cert.subject.cn="<授权公司名>"' \
 --case <案卷>
```
