...
信息收集是攻击链（Kill Chain）的第一步，其质量直接决定后续漏洞发现、利用与横向移动的成败。在实战攻防（红队）与渗透测试中，信息收集的定位已从"找到几个漏洞入口"升级为**攻击面测绘与高价值目标研判**：先把目标"看全"（全资产、全暴露面、全关联），再"看准"（哪些资产最脆弱、最值钱、最容易打穿）。总控闸门：`pentest-methodology`。先被动后主动；版本指纹只是线索，sink 可达才算洞。

**产出物**（写进案卷 `案卷/recon/`，供 `web-vuln-router` / `auth` 卡读取，禁止重复枚举）：`live_hosts.txt` · `urls/` · `js/secrets.json` · `content/` · `screenshots/` · `tech-stack.md`。
...
- 手法：`传承/空间眼.md`
- 工具：`python3 tools/space-search/bin/space_search.py --help`
