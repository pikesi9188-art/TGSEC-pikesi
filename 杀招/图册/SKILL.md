...
5. 本库无此外门 `scripts/render_diagram.py`。图用 Mermaid 写进案卷即可。
...
本库无 `scripts/render_diagram.py`。需要出图时用本机 `mmdc` / Graphviz，或只交 Mermaid 源码。
