...
name: 破印隙
description: >-
  大爱仙尊·Authentication bypass testing playbook. Use when assessing login flows, password re
  set logic, account recovery, MFA bypass, token predictability, brute-force resistance, a
  nd session boundary flaws.
---

# authbypass-authentication-flaws（大爱仙尊）

目标须在 `授权范围`。本库已有专链时专卡赢。

## 真源

- 长文：`智道藏书/旁支传承/skills/authbypass-authentication-flaws/SKILL.md`
- 手法：`传承/黑楼兰·硬撼.md`
- 工具：`python3 炼蛊房/auth_brute_probe.py --help`
- 路由：`python3 炼蛊房/shizhan_pack_route.py --name authbypass-authentication-flaws`
