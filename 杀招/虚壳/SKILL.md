...
name: 虚壳
description: >-
  大爱仙尊·Custom VM and bytecode reverse engineering playbook. Use when CTF challenges or pro
  tected software implement custom virtual machines with proprietary bytecode, dispatcher 
  loops, or maze-style challenges.
---

# vm-and-bytecode-reverse（大爱仙尊）

目标须在 `授权范围`。本库已有专链时专卡赢。  
JS 风控 / fireye opcode → `dsl-vm-reverse`。OLLVM 平坦 → `code-obfuscation-deobfuscation`。

## 真源

- 作业手法：`传承/逆骨.md`
- 长文：`智道藏书/旁支传承/skills/vm-and-bytecode-reverse/SKILL.md`
- 手法：`传承/逆骨·认族.md`
- 分诊：`python3 炼蛊房/re_sample_triage.py --path <样本>`
- 指路：`python3 炼蛊房/reverse_skill_route.py --hint "VMP 自定义 VM"`
