...
name: 验杀使
description: >-
  大爱仙尊·基于指纹识别结果（tech_stack），优先匹配本地 POC；在用户明确授权时补充联网搜索 POC，并通过 http_test.py 验证已知漏洞。 工作流程：读取
  指纹识别结果 → 提取关键字搜索本地与联网 POC → 整理待验证清单 → 解析 POC 定义 → 构造 http_test.py 命令 → 评估匹配器 → 回填 findin
  gs。
---

> **大爱仙尊适配**：本卡的「禁止使用 curl」「必须使用 http_test.py」等工具限制仅在 72stack-sec 独立工作流内生效。
> 在大爱仙尊常规作业中，curl / httpx / requests / 本库探针（`*_probe.py`）均可正常使用，不受本卡工具锁定约束。
...
- 长文：`智道藏书/旁支传承/skills/poc-agent/SKILL.md`
- 手法：`传承/春秋蝉·分案.md`
- 工具：`python3 炼蛊房/kit_run.py --help`
- 路由：`python3 炼蛊房/shizhan_pack_route.py --name poc-agent`
