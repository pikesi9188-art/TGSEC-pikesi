---
name: 秘语
description: 逆向基于 JavaScript 实现的自定义 WASM 虚拟机/风控引擎：VM 识别、Opcode 提取分类、运行时捕获、状态码还原。
---
...
> VMProtect / Themida / 自定义字节码 ISA → `vm-and-bytecode-reverse`。OLLVM / 控制流平坦 → `code-obfuscation-deobfuscation`。
...
- 密钥猎手：`python3 炼蛊房/js_secret_hunter.py file --path <文件> --case <案>`
- 标准 WASM / 商业壳 → `js-reverse` / `vm-and-bytecode-reverse`，不要停在补丁差分卡
