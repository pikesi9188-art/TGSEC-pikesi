...
所有细分查询的结果用 python 脚本合并去重：

```bash
# 把多个 JS fetch 导出的 JSON 数组合并去重（本库 origin_recon.py 可接收 IP 列表）
python3 炼蛊房/origin_recon.py --domain 目标.com --case <案卷>

# 或手动合并去重
python3 - << 'EOF'
import json, glob
all_records = []
for f in glob.glob("quake_*.json"):
...
for r in all_records:
 ...
 seen.add(key)
 unique.append(r)
print(f"合并后 {len(unique)} 条唯一记录")
for r in unique:
 print(f"{r.get('ip','?')}:{r.get('port','?')} {r.get('title','')}")
EOF
```
...
JS fetch API（方式 B） 导航搜索页（方式 A）
 ...
 └─ 方便程序处理 └─ 需人工解析文本
 ↓ ↓
 保存为 JSON 文件 → python3 炼蛊房/origin_recon.py 处理
 ↓
 python3 炼蛊房/origin_recon.py --domain 目标.com --case <案卷>
 ↓
 exports/<案卷>/案卷/ 产出资产清单
```

### 7.2 导出工具（本库实际路径）

```bash
# 域名 + 证书溯源 → 资产清单（origin_recon 封装了 Quake/FOFA/证书查询）
python3 炼蛊房/origin_recon.py --domain 目标.com --case <案卷>

# 把 JS fetch 手动保存的 JSON 快速打印 IP:Port 列表
python3 - << 'EOF'
import json, sys
d = json.load(open(sys.argv[1]))
records = d if isinstance(d, list) else d.get("records", d.get("data", []))
for r in records:
 ip = r.get("ip","?"); port = r.get("port","?")
 title = r.get("title","") or (r.get("service",{}) or {}).get("http",{}).get("title","")
 print(f"{ip}:{port} {title[:60]}")
EOF quake_raw.json
```
...
```bash
# 判断 body 搜索结果是否可信（手动统计标题匹配率）
python3 - << 'EOF'
import json, sys
records = json.load(open(sys.argv[1]))
if isinstance(records, dict): records = records.get("data", records.get("records", []))
target = sys.argv[2].lower()
match = sum(1 for r in records if target in (r.get("title","") or "").lower())
print(f"total={len(records)} title_match={match} rate={match/max(len(records),1)*100:.0f}%")
print("confidence:", "high" if match/max(len(records),1) > 0.6 else ("medium" if match/max(len(records),1) > 0.3 else "low"))
EOF quake_records.json "目标名"
```
