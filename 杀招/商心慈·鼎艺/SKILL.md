---
name: 商心慈·鼎艺
description: "dingyi.io 白标博彩(泰系 Nuxt+megagw CDN)：无验证码注册即返 token、XOR 密码加密逆向、滑块自破、收款账户/TOTP 泄露、KYC 绑卡门槛。触发：webSdk.produce.min/megagw CDN/sso.*.com/VITE_API_BASE。"
version: 1.0.0
license: MIT
...
6. 登录按本卡 XOR 协议手打 / 跟前端 JS 动态 hook；家族分流走 `gambling-family-router`。
...
- 手法：`传承/商心慈·白标.md`
- 工具：`python3 炼蛊房/gambling_family_probe.py --family dingyi --base https://授权站 --case <案卷>`
