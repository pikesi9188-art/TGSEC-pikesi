...
大爱仙尊杀招层：`传承/无底洞.md`。  
授权站确认注入后走本库阶梯，不要停在「存在注入」：

```bash
python3 炼蛊房/sqlmap_kit.py ladder --url 'https://授权/?id=1' -p id --case <案>
```

`waf_sqli_bypass` 优先于裸 sqlmap。dump 授权内直接做，百万拖号先问。对照 `sqlmap-tamper-kit`。
...
- 作业手法：`传承/无底洞.md`
- Tamper：`传承/吞库针.md`
- 工具：`python3 炼蛊房/sqlmap_kit.py ladder --url 'https://授权/?id=1' -p id --case <案>`
