 ...
 **更禁止**把用户主机 / 家宽当成打站出口——漏主机公网 IP。换出口走 `传承/狼烟.md`。
...
```bash
# 本库无此外门 gateway。换出口只走 config/proxy-nodes.txt，禁止把用户主机 SSH 当跳板。
# 旧容器 `--replace` 命令已失效，不要照抄。
```
...
# 3. 本库无此外门 gateway，跳过。浏览器走 chrome-automation。
```
...
# 本库无此外门 profile 目录。新 bot / 注入走 开源版不带助手注入
...
# 7. 本库无此外门 gateway。助手身份走 开源版不带助手注入
```
...
- 工具：`python3 炼蛊房/proxy_classifier.py --help`
