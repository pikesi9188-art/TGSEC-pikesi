...
| 场景 | 用什么 |
|------|--------|
| 授权站抓页 / CF / JS 渲染 | Skill **`scrapling`** |
| 本机有头 Chrome / CDP 9222 | Skill **`chrome-automation`** |
| 会员 Cookie 导入 | `session_pipeline.py` |
| 极验滑块 | `captcha_auto.py` |
| 操作网页（通用 Playwright） | 本卡浏览器段 / agent-browser |
| 操作桌面应用（Windows GUI） | **OpenReverse**（本卡桌面段） |
| JS 断点、Hook、CDP 调试 | Skill `星丝` |
...
- 手法：`传承/凤九歌·天地歌.md`
- 工具：`python3 炼蛊房/cf_session.py --help`
