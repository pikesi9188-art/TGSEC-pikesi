---
name: 窗·使桥
description: "通过 frx-director MCP 驱动 Firefox 浏览器自动化逆向：XHR 拦截、JS 断点、动态 hook、加密协议分析。触发：需要浏览器级动态分析/加密逆向/anti-bot 场景。"
version: 1.0.0
created_by: agent
---
...
本库无此外门 `scripts/frx_mcp_client.py` / `mcp_call.py`。浏览器自动化走 Skill `爬丝` / `browser-automation`。
```
...
- 手法：`传承/星念蛊.md`
- 工具：`python3 炼蛊房/js_secret_hunter.py --help`
