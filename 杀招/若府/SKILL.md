---
name: 若府
description: "若依/飞投/geshanzsq 后台渗透全链：GA-TOTP锁分析+用户名枚举Oracle、druid弱口令(ruoyi/123456全农场)、sql.json业务Schema、Druid盲SQLi验证法、钱包管理变体(wallet.fina.icu)、ddddocr验证码、IP白名单XFF绕过、阿里云WAF acw挑战、旁路资产。🔴禁止密码爆破。"
version: 1.0.0
license: MIT
platforms: [linux]
metadata:
...
>
> **大爱仙尊适配**：此条仅在本卡独立工作流内生效。大爱仙尊 `auth_brute_probe.py` / `strike_probe.py` 短字典弱口探针（如 ruoyi/123456 单条默认凭据）属于 AGENTS.md 授权范围，不受此限。
...
## 3b. Druid 盲 SQLi Oracle（无 RCE 时验证注入面）

> 来源：`ruoyi-fork-admin-pentest`（已并入）

1. **前置**：先抓 `/druid/sql.json` 的 SQL 文本集合 A（hash 去重后的全量集）
2. **发探针**：`/login` 传 `username=' OR '1'='1'--&password=x&code=123456&uuid=...`
3. **对比**：再抓 sql.json 集合 B；B−A 有**新文本** = 参数拼接（注入存在）；无新文本 = 参数化（安全）
4. **注意**：druid 按 SQL 文本 hash 去重——同文本只更新统计，diff 只看**新出现的 SQL 文本**

## 3c. Druid sql.json 业务 Schema（已验证）

`cp_user` 字段（24）：id/ucode/uname/moneys/needmons/botid/parent_botid/iscw/allcz/alltx/allbuy/allpay/backsprop/allwin/allkfjiangmons/uanum/status/cztimes/remark/packcount/fromtype/... 
`sys_user` 含 **google_key 列**（`select google_key from sys_user where user_name = ?`） 
其他表：`cp_busi`/`cp_buys`/`cp_userback`/`cp_change`（钱包审计）/`cp_issue`（开奖）/`cp_config`
...
- 图形码：`python3 炼蛊房/captcha_auto.py ocr`（本库无此外门 `scripts/ddddocr_login_brute.py`；授权内短字典默认可跑，勿当无限爆破）
...
## 9. 钱包管理系统变体（wallet.fina.icu / 43.156.67.243）

> 来源：`ruoyi-fork-admin-pentest`（已并入）

- **发现方法**：对前端服务器做 Host header fuzz（2042 个 `prefix.domain` 组合）——未绑定 Host 落到**默认 vhost**。154.9.25.34 默认 vhost = 钱包管理系统（Vben SPA）。**任何服务器先测裸IP/未绑定Host的默认vhost**
- API：`wallet.fina.icu/prod-api`（43.156.67.243）— 飞投 v3.9.0 **新版**，防护差异：
 - `/sys/*` `/mer/*` 无 token → **静默空 200**（Content-Length:0；假 token 也空 200）；其他路径 JSON 401
 - druid/actuator 全移除（404）；swagger 页面 200 但 api-docs 401
 - 错 GA 5 次锁 3 分钟；`登录账号不存在` 在 GA 检查**之前**（枚举更直接）
 - 用户：admin + sysadmin（均 GA）；41 个弱码（000000/888888/123456/314159/271828...）全失败
- `_app.config.js` 泄露**完整 RSA 私钥**（VITE_GLOB_RSA_PRIVATE_KEY，与 ht.crypto777.vip 同密钥对；ENABLE_ENCRYPT=false 未实际使用）

### 业务端点（cp/* 全需 token）

`/sys/cp/user/*`：dodraw/backDeposit/paysuc/setPwd/**setGoogleKey**/resetPwd/resetaddr/setFreeTxCount/getUsersByDevice 
`/sys/cp/withdraw/auditList`、`/sys/cp/transfer/retryCallback`、`/sys/cp/redpack/refund` 
`/sys/cp/subWallet/*`、`/sys/cp/flashExchange/*`、`/sys/cp/key/*`、`/sys/merchant/*` 
`/mer/cp/com/*`（dodraw/depositmoney/transferMoneys）、`/sys/ls/*`（eSIM）、`/withdrawalReview`
...
- 图形码：Skill `白兔·认纹` · `炼蛊房/captcha_auto.py`
