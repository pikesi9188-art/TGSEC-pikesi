...
name: 注门使
description: >-
  大爱仙尊·负责所有注入类漏洞检测：SQLi/NoSQL/XSS(存储/反射/DOM)/SSRF/XXE/SSTI/RCE/反序列化/CRLF/XSLT/EL/JNDI
---
...
# injection-agent（大爱仙尊）

目标须在 `授权范围`。本库已有专链时专卡赢。

## 真源

- 长文：`智道藏书/旁支传承/skills/injection-agent/SKILL.md`
- 手法：`传承/春秋蝉·分案.md`
- 工具：`python3 炼蛊房/kit_run.py --help`
- 路由：`python3 炼蛊房/shizhan_pack_route.py --name injection-agent`
