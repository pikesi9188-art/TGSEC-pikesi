...
name: 罩辨
description: >-
  大爱仙尊·Web WAF 检测与绕过工具。检测目标 WAF 类型并提供绕过策略。Invoke when user mentions WAF检测、WAF识别、WAF bypass
  、绕过WAF、检测防火墙类型、SQLi/XSS payload 测试.
---
...
- 长文：`智道藏书/旁支传承/skills/waf-detector/SKILL.md`
- 手法：`传承/隐鳞·手册.md`
- 工具：`python3 炼蛊房/waf_detect.py --help`
- 路由：`python3 炼蛊房/shizhan_pack_route.py --name waf-detector`
