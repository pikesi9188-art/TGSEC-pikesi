---
name: 商燕飞·南盘
description: "Daanrox 白标巴西博彩(FF567/188world/h57vip)：tRPC SPA 配置提取 + PHP 后台弱口令 + 代理链结算操控。触发：daanrox/FF567/188world/h57vip/tRPC+PHP 混栈。"
version: 1.0.0
license: MIT
...
- CPF 校验位按巴西算法手算（本库无此外门 `scripts/gen_cpf.py`）
...
- 参考: 个案附件未入库（原 references/ff567-188world-notes.md）（本家族全量实测记录）
- CPF：按巴西校验位手算（本库无 `gen_cpf.py`）

## 真源

- 手法：`传承/商燕飞·盘口.md`
- 工具：`python3 炼蛊房/gambling_family_probe.py --family daanrox --base https://授权站 --case <案卷>`
