---
name: 铁若男·拆骨
description: Android + iOS 移动应用逆向与安全测试方法论，覆盖 APK/IPA 逆向、Frida/Objection 动态插桩、SSL Pinning 绕过、Root/越狱检测绕过、移动端加密算法提取。
 客户端渗透、组件导出、WebView、allowBackup、手势密码。
 Android 客户端全表走 传承/安器·清单.md；
 博彩情报优先 apk-recon。
---
...
- 作业手法：`传承/逆骨.md` §4
- Android 清单：`传承/安器·清单.md`
- iOS 应用面：Skill `白凝冰·探府` · `传承/iOS渗透测试/`
- 情报：`python3 炼蛊房/apk_recon.py extract --apk <授权.apk> --case <案>`
- 分诊：`python3 炼蛊房/re_sample_triage.py --path <apk|ipa>`

仓库没有 `mobile-reverse/references/`，不要找。IPA 不要当 macOS `.app`。内核/固件走 `ios-research/`，不走本卡。
