---
name: 使桥
description: "MCP 工具箱按需调用路由：chrome-devtools（动态 hook/XHR 拦截/DOM 分析）+ jshook（JS 运行时 hook/加密逆向）+ scrapling（CF 绕过爬虫）。触发：需要浏览器自动化/JS hook/动态加密逆向/anti-bot 绕过时使用。"
---
...
```bash
# 本库无此外门 gateway。浏览器自动化走 chrome-automation / scrapling，不要抄 此外门 mcp 命令。
python3 炼蛊房/kit_run.py --help
```
