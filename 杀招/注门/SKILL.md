...
name: 注门
description: >-
  大爱仙尊·Expression Language injection playbook. Use when Java EL, SpEL, OGNL, or MVEL expre
  ssions may evaluate attacker-controlled input in Spring, Struts2, Confluence, or similar
   frameworks.
---

# expression-language-injection（大爱仙尊）

目标须在 `授权范围`。本库已有专链时专卡赢。

## 真源

- 长文：`智道藏书/旁支传承/skills/expression-language-injection/SKILL.md`
- 手法：`传承/薄青·岁岁索命.md`
- 工具：`python3 炼蛊房/rce_forge.py shoot --family el-spel --url 'https://授权/' --param q --case <案>`
- 探测：`python3 炼蛊房/core_web_surface_probe.py --help`
- 路由：`python3 炼蛊房/shizhan_pack_route.py --name expression-language-injection`
