---
name: 太白云生·芋府盘
description: "芋道系 TG Mini App 博彩(YuDao/ruoyi-vue-pro)：app-config.js 提取加密密钥、sk_encrypt 逆向、initData 校验绕过、充值通道探测。触发：app-config.js/sk_encrypt/yudao TMA/芋道博彩小程序。"
version: 1.0.0
license: MIT
platforms: [linux]
metadata:
...
the SAME class as the Qzino family (see `telegram-tma-gambling` in the default profile for the
...
6. 本库无此外门 `scripts/chuangshi_client.py`。实战入口：`python3 炼蛊房/yudao_appapi_probe.py`。
...
## References
- 个案附件未入库（原 references/chuangshi-endpoint-map.md） — full endpoint list + observed responses + attack results (2026-08)
- 本库客户端：`python3 炼蛊房/yudao_appapi_probe.py`（无此外门 `chuangshi_client.py`）
