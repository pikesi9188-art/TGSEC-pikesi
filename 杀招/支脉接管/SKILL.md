...
name: 支脉接管
description: >-
  大爱仙尊·Subdomain takeover detection and exploitation playbook. Use when targets have dangl
  ing CNAME/NS/MX records pointing to deprovisioned cloud resources, expired third-party s
  ervices, or unclaimed SaaS tenants that an attacker can register to serve content under 
  the victim's domain.
---

# subdomain-takeover（大爱仙尊）

目标须在 `授权范围`。本库已有专链时专卡赢。

## 真源

- 长文：`智道藏书/旁支传承/skills/subdomain-takeover/SKILL.md`
- 手法：`传承/空间眼.md`
- 工具：`python3 炼蛊房/origin_recon.py --help`
- 路由：`python3 炼蛊房/shizhan_pack_route.py --name subdomain-takeover`
