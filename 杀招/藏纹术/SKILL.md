...
name: 藏纹术
description: >-
  大爱仙尊·Steganography detection and extraction playbook. Use when analyzing images (LSB, PN
  G chunks, JPEG DCT, EXIF), audio (spectrogram, DTMF), files (polyglots, appended data, A
  DS), and text (whitespace, zero-width, homoglyphs) for hidden data.
---

# steganography-techniques（大爱仙尊）

目标须在 `授权范围`。本库已有专链时专卡赢。

## 真源

- 长文：`智道藏书/旁支传承/skills/steganography-techniques/SKILL.md`
- 手法：`传承/使证闸.md`
- 工具：`python3 炼蛊房/crypto_decode.py --help`
- LSB 探针：`python3 炼蛊房/stego_lsb_probe.py drive --path img.png --case <案>`
- 路由：`python3 炼蛊房/shizhan_pack_route.py --name steganography-techniques`
