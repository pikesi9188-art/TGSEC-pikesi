...
> 旧名 `quant-copy-trading-pentest` 已重定向到本卡，勿再展开旧正文。
...
### P4 数据提取 (达标即交付)
- 本库无此外门 `scripts/dump_trade_dashboard.py`。按本卡端点用 curl 分页拉 `trade-dashboard`，落案卷 JSON/CSV。
...
- 手法：`传承/商燕飞·盘口.md`
- 工具：`python3 炼蛊房/gambling_family_probe.py --family quant --base https://授权站 --case <案卷>`
