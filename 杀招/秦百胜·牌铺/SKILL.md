...
> 旧名 `faka-shop-pentest` 已重定向到本卡，勿再展开旧正文。

# 发卡/卡商/多店商城渗透 (faka card-shop pentest)

类级 umbrella：覆盖中国发卡网/卡密商城/海外号批发站(云发卡Toka、独角数卡系、表哥团队等变体)。
侧重**全站混淆壳解码**+**无鉴权订单IDOR**+**多店共库回调**；CJSC/Laravel 订单流见旧卡 `faka-shop-pentest/ARCHIVE.md`。
...
- 卡密常见在 `/user/api/index/secret`(orderId+password)。**它先验支付状态**(未付=「该订单还未支付」)再验密码 → 未付单永远拿不到卡密，要卡密必须先把订单变成已支付(回调伪造/后台)。
- **版本差（ACG 3.4.8 实锤）**：部分站 `query` 对**已付单**直接回 `secret` 字段（邮箱/订单号即可）。未付仍不出卡。测到则并行记「无鉴权查单泄卡」，主链仍是假付或支付栈取钥。

## 2. 回调跨域 = 同后端多店 (横向线索)
- 易支付/彩虹 submit 表单里 `notify_url` 指向**另一个域名**而当前站在 A 域 → A、B 同套 `/user/api` 结构、**共库多店租户**。对共债 host 打出的回调/越权可跨店作用于本店订单。
- 回调验签通常严(错 sign → `sign error`)。⚠️ 商户密钥**爆破已作废**（用户红线：禁止任何暴力破解/字典穷举，2026-08 纠正）——拿到 (参数+sign) 对只用来**验证算法一致性**（把 submit sign 直打 notify，若返回 success 则算法吻合），不穷举 key。严签后的换面是打**支付栈 admin 取钥**（`宝黄天·回灌.md` · `payment-callback-forgery`），不是穷举 pid。
...
## 8b. 日志泄商户钥 → 合法签（F-01，先于支付栈）

PHP/Laravel 发卡（`composer.json` 200）严签矩阵阴 **不算结案**。  
`sensitive-dir-dump` 打 `/storage/logs/`、`laravel.log`、`/runtime/logs/`：异常栈常带 `merchant_pem` / epay `key`。抽到钥 → `pay_matrix --key` / 假支付 §4.4 合法签 → 未付探针单出卡。钥只进案卷。  
并行 F-02：已付单游客查单回 `secret`/`info`（§9）。F-03 状态口若区分 paid/pending 可辅助找已付 SN，**禁止批量枚举**；admin 无验证码走 `auth-brute`。F-04：3306 MySQL 5.7 / SSH 22 走 `middleware_unauth`，写库先问。
