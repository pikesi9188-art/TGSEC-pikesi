...
```
Step 1: Victim server resolves test-attacker.com → gets 1.2.3.4 (legitimate IP)
Step 2: Server validates: "1.2.3.4 is not internal → allowed"
Step 3: DNS TTL expires (or attacker uses TTL=0)
Step 4: Server makes actual request → resolves test-attacker.com again → gets 169.254.169.254
...
# 1. Victim visits test-attacker.com (DNS resolves to attacker IP)
# 2. JavaScript makes request to test-attacker.com (same-origin)
