...
### 11.1 MadeYouReset — CVE-2025-8671 (disclosed Aug 13, 2025, Tel Aviv University)

Distinct from Rapid Reset (CVE-2023-44487). Rapid Reset abuses **client-sent** RST_STREAM; MadeYouReset abuses **server-sent** stream resets. The attacker deliberately sends frames that trigger a protocol violation, inducing the server to reset the stream — and the open+reset cycle still consumes server resources, producing a DoS asymmetry.

```
Rapid Reset:    client sends HEADERS, client sends RST_STREAM  (client initiates reset)
MadeYouReset:   client sends a malformed/illegal frame → server detects violation
                → server sends RST_STREAM  (server initiates reset, but still pays the cost)
```

Affects unpatched HTTP/2 implementations with limited mitigation; the Rust **h2 crate < 0.4.11** is vulnerable. Cloudflare was already immune due to its 2023 Rapid Reset mitigations.

### 11.2 H2.CL / H2.TE Desync (2026 desync primary battleground)

Any architecture where the front end (Cloudflare / Fastly / AWS ALB) receives HTTP/2 and the back end is forwarded HTTP/1.1 is on the risk surface.

- **H2.CL**: the H2 request declares `content-length: 0` but the DATA frame length covers a full body. The front end trusts the frame length; on downgrade it emits the literal `Content-Length: 0` header. The back end reads 0 bytes and treats the trailing body as the next request.
- **H2.TE**: HTTP/2 forbids `transfer-encoding`, but some front ends forward it on downgrade. The front end ignores TE (uses frame length); the back end honors `chunked` and a `0\r\n\r\n` terminator splits off a smuggled request.

See `../request-smuggling/H2_SMUGGLING_VARIANTS.md` for byte-level payloads.

### 11.3 H2 CRLF Header Injection

HTTP/2 headers are binary and may legally contain bytes that are illegal in HTTP/1.1 header values, including `\r\n`. On downgrade these bytes become real line breaks → a complete second request can be smuggled inside a single header value.

### 11.4 HTTP/2 Rapid Reset — CVE-2023-44487 (retrospective)

Disclosed October 2023. The attacker repeatedly opens an HTTP/2 stream (HEADERS) and immediately resets it (RST_STREAM), never waiting for a response. Cloudflare reported a peak of **201M rps**; AWS recorded **155M rps**. The record-setting DoS relied on the asymmetry between cheap client-side open/close and expensive server-side stream-lifecycle bookkeeping. MadeYouReset (§11.1) is the 2025 successor that flips the reset direction.

### 11.5 HTTP/2 Continuation Flood (2026 复现与变种)

CONTINUATION帧无限制累积导致的DoS。攻击者发送HEADERS后持续发送CONTINUATION帧（不发送END_HEADERS），服务端必须缓存所有头部直到帧流结束，内存耗尽崩溃。

```
攻击: HEADERS(stream=1, END_STREAM, 无END_HEADERS)
      CONTINUATION(stream=1) × N  (每个64KB头部, 永不结束)
      → 服务端缓冲区溢出 → OOM崩溃
```

**2026变种**：
- **跨流Continuation洪泛**：在多个stream上同时发起未结束的CONTINUATION，绕过单流限制
- **混合H2.CL+Continuation**：利用H2.CL反同步让CONTINUATION帧被错误解析
- **影响面**：h2 crate < 0.4.11、nghttp2 < 1.62、Go net/http2（2026补丁）

### 11.6 HTTP/3 (QUIC) 攻击面 (2026 新主战场)

HTTP/3基于QUIC（UDP），引入全新攻击面：

| 攻击向量 | 原理 | 2026现状 |
|---------|------|---------|
| **0-RTT重放** | QUIC 0-RTT早期数据可被重放 | 连接迁移劫持利用此特性 |
| **连接迁移劫持** | QUIC允许迁移连接ID，攻击者劫持新路径 | ECH降级对抗 |
| **加密握手指纹** | QUIC TLS指纹可被识别封锁 | 指纹混淆对抗 |
| **CID预测** | 连接ID可预测则可注入数据包 | 随机CID缓解 |
| **放大攻击** | UDP反射放大，QUIC响应大于请求 | 地址验证缓解 |
| **ECH绕过** | Encrypted Client Hello隐藏SNI，WAF无法识别 | 部分WAF盲区 |
| **QUIC感知DPI** | 识别QUIC流量特征（CID/初始包）进行封锁 | 流量整形对抗 |

**0-RTT重放实战**：
```text
1. 攻击者捕获受害者的0-RTT早期数据(含POST请求)
2. 在新连接上重放该数据
3. 服务端处理重复请求 → 重复支付/重复操作
4. 防御: 应用层幂等性 + 禁用0-RTT for非幂等方法
```

### 11.7 H2.SMUGGLING 2026 高级变种

| 变种 | 原理 | 检测特征 |
|------|------|---------|
| **Content-Length帧长度混淆** | H2声明content-length:0但DATA帧有body | 前端信任帧长，后端信任CL |
| **伪HEADERS注入** | 注入`:method`/`:path`伪头部 | 后端重复解析伪头部 |
| **DATA帧边界攻击** | 残缺DATA帧 + 下个请求拼接 | 帧长度与实际body不符 |
| **优先级帧滥用** | PRIORITY帧操纵流优先级致DoS | 大量低优先级流饿死 |
| **SETTINGS帧洪水** | 持续发送SETTINGS帧强制对端重配 | DoS无RST_STREAM特征 |
| **WINDOW_UPDATE操纵** | 操纵流控窗口致死锁或内存耗尽 | 流控状态不一致 |

### 11.8 2026 H2攻击检测信号矩阵

| 攻击 | 检测信号 | 工具 |
|------|---------|------|
| H2.CL走私 | 前端与后端响应时间差 | httpx-smuggler |
| Continuation Flood | 单流CONTINUATION帧计数 | nghttp2监控 |
| MadeYouReset | 服务端发起的RST_STREAM比率 | h2 crate日志 |
| 0-RTT重放 | 相同0-RTT数据多连接出现 | QUIC流量分析 |
| 伪头部注入 | 重复:method/:path头 | H2解析器告警 |
