...
> Companion to [SKILL.md](./SKILL.md), [SCENARIOS.md](./SCENARIOS.md), [METHODOLOGY.md](./METHODOLOGY.md). Distilled from instructor-led classes and original business-logic vulnerability training materials, every entry below has a **why** (root cause) and a **verify** (how to reproduce).
...
| 验证码不可在响应中明文回传 | 案例：JSON `{"randm":9759,"code":0}` 直接给出 | 抓包搜索响应体里是否含 `code`/`randm`/`captcha` 等字段值与短信内容一致 |
...
| 公众号 / 小程序 / 内部接口同样有验证码 | "只有公众号才管的入口"是高风险点 | 试访问 H5 页未暴露但 App 内有的接口 |
...
| Item | why | verify |
|---|---|---|
| 重置 token 不可基于弱随机数 | `Windows rand()` max=32768 可枚举 | 看后端 token 生成是否 `md5(rand())`；本地 0~32768 全量字典爆破对照 |
...
| 短信验证码不能写在 Cookie / 响应包 | 验证码出现在前端可见位置就等于泄露 | 抓包搜索响应体 `"randm"`、`"verify_code"` |
...
| 高并发下不能绕过频率限制 | 简单计数器有竞态 | 用 Burp Turbo Intruder `concurrentConnections=30` 同时发，看是否突破限频 |
...
| 验证码不可在响应里 echo | 响应体回显验证码会直接绕过验证 | 搜索响应体里是否包含与短信一致的数字 |
| 同一验证码不可多次使用 | 否则可重放 | 输入正确码，重复 submit N 次看是否成功 |
| 长度足够（建议 6 位+）防爆破 | 4 位纯数字 5 分钟可爆破完 | 用 Burp Intruder 0000~9999 全量爆破计算耗时 |
| 不存在"万能码"残留 | 开发/测试遗留弱验证码可被直接利用 | 尝试 `000000`、`123456`、`888888`、`111111` |
...
| 支付状态由后端根据网关回调写，不接受客户端 `payment_status=paid` | 客户端可控支付状态会被伪造 | 改 `payment_status=paid`、`is_paid=1` 看订单是否被标记完成 |
| 数量字段必须为正整数 | `quantity:-1` / `quantity:0.02` 都构成漏洞 | `skuQty:0.02`、`FoodNum:0.01` |
| 数量字段做上限校验防整数溢出 | int32 max+1 = -2147483648 | 提交 `quantity=2147483648` 看是否回绕 |
| 优惠券面值不能为负数或大于商品价 | `coupon_amount=-100` 反加；`coupon_amount > price` 直接 0 元 | 抓包修改 coupon 字段 |
| 即使前端报"支付失败"，订单详情里 amount 必须等于商品价 | 订单可能已生成，再用站内钱包补支付 | 改券后看订单详情，尝试用钱包支付 |
| 支付接口名 / 渠道走白名单校验 | 否则替换不存在接口可"成功" | 改 `payment_method=invalid_chan` 看响应 |
| 试用接口与购买接口完全隔离 | 否则"试用 URL 改成购买"刷免费 VIP | 把 URL 末尾的 `4` (试用) 改 `3` (购买) 看是否仍归 0 元 |
| 收款人 `username`/`account_id` 由服务端 session 决定 | 否则可替换为受害者扣他钱 | 改 `username=victim` 看是否扣他人余额 |
| 充值金额精度不被四舍五入吃掉 | 0.019 元只扣 0.01 但钱包加 0.02 | 充 0.019 看实际扣款与钱包变化 |
...
| 多设备并发签约不获新人优惠 | 服务端必须 lock 用户状态 | 多手机同时进新人优惠支付页，依次完成 |
| 限时活动用服务端时间，不信前端 timestamp | 否则改 time 绕过 | 改请求里 `time` 参数为活动外时间 |
| 关键字段服务端有签名 / HMAC，签名覆盖**所有**影响金额参数 | 否则未签名间接字段（数量、折扣）改了仍生效 | 对比正常 vs 篡改请求，找未签名字段 |
...
| Item | why | verify |
|---|---|---|
| 单券一次性 + 状态绑定订单 | 否则订单关闭后券仍在，重复使用 | 创建订单 → 关闭 → 重新下单看券是否仍可用 |
...
| 抽奖次数 / 概率不可被前端控制 | 抽奖参数可控即可刷奖 | 抓包改 `chance_count`、`prize_id` |
...
| 数量为负 / 0 / 浮点都被拒绝 | 负数/零/浮点数量会破坏金额与库存逻辑 | `quantity=-1`、`quantity=0`、`quantity=0.02` |
...
| 退货/退款不能跳过商家审核 | 可绕过退款审核流程会直接造成损失 | 直接调 refund API 不走审核 |
...
| Item | why | verify |
|---|---|---|
| 实名认证后认证状态锁定，不可"故意填错驳回再修改" | 故意填错身份证后若可重置入口，可绕过状态锁定 | 用错误身份证提交认证，被驳回后看是否还能改身份信息 |
| 提交错误身份证 N 次自动锁定或人工审核 | 否则可重放绕过防沉迷 | 连续 5 次提交不同错误身份证看是否被风控 |
| 服务端响应不能信前端 success 字段 | 前端信任响应字段可被改包绕过 | Burp 改响应 `success:false → true` 看前端流程是否放行 |
...
| 数据脱敏跨接口一致 | 不同接口掩码不一致可被拼接还原，例如 A 显 `138****5678`，B 显 `1384***5678`，C 显 `13845**678` | 列出所有泄露同一字段的接口对比掩码方式 |
| 敏感字段不在响应里全量返回 | 用户信息查询返回 password / id_card 全文 | 抓登录 / 个人中心查询响应，搜 `password`、`身份证` |
| 接口枚举有频率限制 + 行为分析 | 不同状态码或响应差异会暴露身份证号有效性 | Burp 用递增身份证号枚举，看响应差异（length/code） |
...
| Item | why | verify |
|---|---|---|
| 资源链接（视频/音频/课程文件）必须服务端鉴权，不能在响应里给直链 | 抓包发现 `previewPullUrl: ...flv` 直链时，常可直接下载 VIP 资源 | 抓详情接口响应，搜 `.flv`、`.mp4`、`.m3u8`、`url`，复制到 VLC 或 flv.js demo 播放 |
...
| VIP 时长升级支持幂等防重 | 多端并发补差价升级被锁 | 多设备同时点"补差价升级"完成支付看 VIP 时长是否多倍叠加 |
| 试用 → 购买流程隔离 | 试用接口换接口号即获购买 | URL 末尾 `/4` 改 `/3` 看是否仍按试用价 |
...
| 校验需覆盖编码绕过 | `?`、`@`、`/`、`#`、子域名、畸形 URL、IP 直连等多种手法 | 提交 `https://target.com@evil.com`、`https://target.com.evil.com`、`https://evil.com/?target.com`、`https://evil.com#target.com`、`https://[::1]/`、`https://0x7f000001/` |
...
| 第三方账户接口未授权 | 第三方系统接口若未鉴权会直接暴露 | 删 cookie 访问 /third/api/* |
...
| 改 cookie 字段不能影响权限 | 客户端可控权限字段会导致越权 | 改 `JSESSIONID`、`uid` 后看响应变化 |
| 重置 cookie 不会重置服务端验证码计数 | 否则删 cookie 即解封 | 删 cookie 重新请求验证码看是否再次成功 |
...
| 限频短信验证码 | concurrentConnections=30, requestsPerConnection=10 |
| 多端并发会员升级 | 多设备同时触发升级流程 |
...
> 仅针对"上传业务"自身的逻辑校验。若需要继续覆盖上传验证、落地路径、预览、解析链与跨技能利用，请联动 `../file-access-vuln/SKILL.md`，并按场景继续查看 `path-traversal-lfi`、`xxe-xml-external-entity`、`xss-cross-site-scripting`、`cmdi-command-injection`。
...
参考：[SKILL.md §8](./SKILL.md)、[../file-access-vuln/SKILL.md](../file-access-vuln/SKILL.md)。
