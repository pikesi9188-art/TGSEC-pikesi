...
**真实案例**：修改手机号场景，服务端返回 `{"error_msg":"旧密码错误","error_code":99999,"success":false}`，攻击者把响应改成 `{"success": true,"error_code": 1}`，前端"看到 success 就放行"导致绕过密码校验直接换绑手机号。**根因是客户端过度信任服务端响应里的状态字段**，必须在服务端独立持久化"该用户是否已通过密码验证"。
...
一个经典例子是：找回密码 URL `resetpassword.php?id=MD5`，如果 auth 是用 PHP 老版本 Windows 下的 `rand()`（最大值 32768）生成 md5，攻击者可以本地遍历 0~32768 全量计算 md5 字典，**整个找回密码机制相当于失效**。
...
| 案例 | 矩阵位置 | 出处 |
|------|---------|------|
| 删除 `prizeIdList` 字段实现 0 元购 | 参数篡改 × 支付 | 活动支付案例 |
| `quantity:0.02` 实现半价 | 参数篡改 × 订单 | 商城数量精度案例 |
| `quantity=999999999` 整数溢出归零 | 参数篡改 × 支付 | 整数溢出案例 |
| Burp Turbo Intruder `concurrentConnections=30` 绕过短信限频 | 并发 × 验证码 | 高频并发限频绕过案例 |
| 删除 `JSESSIONID` 重置验证码次数 | 参数篡改 × 验证码 | 会话绑定缺失案例 |
| 修改响应 `success:false → true` 绕过改手机号 | 参数篡改 × 隐私 | 前端信任响应状态案例 |
| 故意填错身份证使审核驳回，反复修改实名 | 流程跳跃 × 隐私 | 实名状态机绕过案例 |
| 抓包发现 `previewPullUrl: *.flv` 直链下载 VIP 资源 | 参数篡改 × VIP | 付费资源直链暴露案例 |
| 多设备并发签约领新人优惠 | 并发 × VIP | 新人优惠并发滥用案例 |
...
法律红线需要在团队培训中单独强调，**任何团队培训时都应该带着读一遍**：
