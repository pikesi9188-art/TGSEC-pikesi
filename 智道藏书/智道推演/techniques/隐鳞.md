---
name: evasion
description: 大爱仙尊·假设作业长文（evasion）。先专卡后读本文。
---

> 长文只讲假设。落地走 `edr-bypass-re` + `payload_forge.py campaign` + `se_listen.py` + `host_c2_verify.py`。  
> **别人家授权 C2 控制台 / 敲门 / 黑洞**走 `c2-zero-trust-console` · `c2_zt_probe.py`，不要并进本机 Sliver 落地。
...
3. 始终在测试环境先验证 —— EDR 版本差异可能使某些技术完全失效
...
## 2026 内核级调用栈伪造（2026 增量）

> 与上文"硬件断点调用栈欺骗"互补——这些技术不修改任何代码段字节，纯地址/寄存器利用，CrowdStrike/SentinelOne/MDE 均无法通过栈帧检测。

### BYOUD-Gap 调用栈伪造
- **信号**: EDR 拦截传统 shellcode loader，栈回溯检测恶意调用链
- **验证**: 扫描 kernel32/ntdll 中未使用但 CET 兼容的代码间隙 → 构造调用栈（代码→kernel32 间隙→ntdll syscall）→ EDR 内核回调看到的栈帧全部在合法模块范围内
- **独特点**: 零修改 CET 兼容，不修改任何模块代码段，纯地址利用

### VEH LayeredSyscall
- **信号**: ntdll inline hook 被 EDR 监控
- **验证**: 注册 VEH 处理器 → 在目标 API 入口设硬件断点（DR0-DR3）→ 触发 EXCEPTION_SINGLE_STEP → VEH 提取 SSN 并直接执行 syscall → 全程不修改目标 API 字节
- **独特点**: 硬件级透明代理，EDR 内存扫描完全无法检测，比传统间接 syscall 更隐蔽

### Process Ghosting（进程幽灵化）
- **信号**: 需要隐蔽进程注入，避免"文件落地"检测
- **验证**: 创建临时文件 → 写恶意 PE → NtSetInformationFile 标记 Delete-Pending → 从标记删除的文件创建 Section → 创建进程 → 关闭句柄（文件从磁盘消失）→ EDR 扫描时文件不存在
- **独特点**: 解决文件落地检测，CrowdStrike/SentinelOne/MDE 可绕过

### P³（Process Parameter Poisoning）
- **信号**: 需要绕过 VirtualAllocEx + WriteProcessMemory + CreateRemoteThread 检测模式
- **验证**: 创建挂起进程（大命令行缓冲区）→ 读取 PEB → CommandLine.Buffer → 将 shellcode 写入缓冲区 → 修改 RIP → 恢复线程
- **独特点**: 不使用传统注入 API 组合，CrowdStrike/SentinelOne/MDE/Cortex 四 EDR 均无法检测

### Pool Party 线程池注入（8 种变体）
- **信号**: NtCreateThreadEx 被监控，需要不创建新线程的注入
- **验证**: 覆写 TP_WORK/TP_IO/TP_WAIT/TP_ALPC/TP_TIMER/TP_DIRECT/TP_JOB/TP_CLEANUP_GROUP → 复用系统线程池执行
- **独特点**: 绕过内核线程创建回调，8 种变体增加 EDR 检测难度

### 国产三巨头专项绕过（360/火绒/电脑管家）
- **信号**: 目标装 360/火绒/电脑管家
- **验证**: 360（QVM/云查杀/启发式 → 低熵 XOR + LLM 语义重写）、火绒（行为分析+系统加固 → 行为分片 + 时间分散）、电脑管家（多引擎+云查杀 → PE 头伪造 + 特征变异）
- **独特点**: 国内实战刚需，覆盖国产杀软特定检测机制

### 18 维反沙箱加权评分系统
- **信号**: 需绕过沙箱分析（CPU 核数/内存/磁盘/运行时间/鼠标轨迹/反调试/注册表 VM 键/进程数）
- **验证**: 18 维加权评分，总分 ≥8 → 延迟 60 秒退出；含鼠标三角轨迹检测区分真实鼠标移动
- **独特点**: 具体阈值+加权值可直接代码化

### 18 维 Shellcode 处理流水线
- **信号**: 需要生成免杀 payload
- **验证**: 指令级 Patch（17 种花指令）→ 4 层加密（ChaCha20+AES-256-GCM+RC4+自定义 SM4）→ 8 种格式混淆（UUID/IPv4/IPv6/MAC/GUID/Base64/Word/Decimal 分块随机化）
- **独特点**: 分块随机化（每块不同格式+随机打乱），卡巴斯基分块 RC4 密钥轮换模式，360 QVM 低熵 XOR 模式

### 休眠混淆变体（Ekko/Zilean/Foliage）
- **信号**: EDR 内存扫描检测 shellcode，睡眠时扫描
- **验证**: XOR 加密 shellcode → CreateWaitableTimer 精确延迟（Ekko）/ SetTimer（Zilean）/ SleepEx+APC（Foliage）→ 定时器到期后解密 → 恢复 RX 权限
- **独特点**: 三种变体覆盖不同 EDR 检测场景

---

## 痕迹清理 / 反取证

> 核心认知：**清理动作本身产生新日志**，EDR 时代是"减少证据"而非"无影无踪"。优先做低噪音清理。

### Linux 痕迹清理
- **验证**: 登录日志 `> /var/log/utmp` / `> /var/log/wtmp` / `> /var/log/btmp` / `> /var/log/lastlog`（清空会破坏二进制格式，用 logtamper.py 定向清理更隐蔽：`-m 1` 躲 `w` 查看、`-m 2` 清指定 IP 的 lastb、`-m 3` 改 lastlog 时间）→ 不记录 history `unset HISTFILE; export HISTFILE=/dev/null` → 清理 bash_history / `.bashrc` 注入痕迹
- **注意**: 清空日志是双刃剑——空 wtmp 反而可疑，定向删除自己那几条更佳

### Windows 痕迹清理
- **验证**: 事件日志 ID 表——4624(登录成功)/4625(登录失败)/4634(登出)/4720(建用户)/4732(加组)/7045(服务安装)/4103+4104(PowerShell 脚本块最危险)/Sysmon 1(进程创建)/3(网络)/11(文件创建) → `wevtutil cl Security`（会产生 1102"日志已清除"强告警，慎用）→ 更干净做法：`Invoke-Phant0m` 挂起 EventLog 线程，不产生 1102 → RDP 记录 `HKCU\...\Terminal Server Client\Servers` → 执行痕迹 Prefetch/ShimCache/Amcache/Recent/JumpLists → `timestomp` 伪造文件时间戳
- **注意**: 1102 事件本身就是强告警信号——优先定向删自己相关事件而非全清

### 获取 Linux 哈希（strace 抓 sshd 明文）
- **验证**: `/etc/shadow` 三字段 `$id$salt$encrypted`（1=MD5/5=SHA256/6=SHA512，john 爆破）→ **strace 调试 sshd 抓明文**: `strace -f -p $(pgrep "sshd -D") -e trace=read,write` → grep `read(6,...` 截获登录密码（冷门高价值手法）→ 替换 pam.so 抓 ssh 密码（条件苛刻）
- **升级**: 明文密码 → 横向移动/复用

### WebShell 免杀排查
- **分面**: 落地静态特征（文件内容/YARA/AV 扫描）vs 通信流量特征（请求/响应模式）→ 两部分分开处理
- **站位**: 命令执行问题和没有回显是两回事——用落文件验证重定向链路（如 `curl -o /tmp/test.txt http://server/file` 确认出网）
- **Windows 不执行排查**: 切解释器（aslistcmd/ascmd）、cmd.exe 被替换/PowerShell 版本兼容/杀软拦子进程
- **Linux 不执行排查**: `disable_functions`、SELinux/AppArmor、sh/bash 路径问题

---

## macOS 安全绕过

> 仅限明确授权的 macOS 测试任务。目标是 macOS 主机/TCC 资源时进入。

### TCC 绕过（macOS 权限框架）
- **信号**: 已获 macOS 代码执行，需访问相机/麦克风/通讯录/全盘访问等 TCC 保护资源
- **验证**:
  - 查 TCC 库: `sqlite3 ~/Library/Application Support/com.apple.TCC/TCC.db "SELECT service,client,allowed FROM access;"`（需 FDA 或 SIP 关）
  - **FDA 继承**: Terminal/iTerm 有全盘访问 → 其子进程继承 → 直接读 `~/Library/Messages/chat.db`、邮件索引
  - **Finder 自动化**: 自动 Finder（权限门槛更低）访问受保护文件 `tell application "Finder" to duplicate f to desktop`
  - 系统设置注入（写 Application Scripts 文件夹）、MDM PPPC 配置滥用
- **升级**: 全盘访问 → 数据窃取（chat.db/邮件/钥匙串）

### Gatekeeper / SIP 绕过
- **验证**: `xattr -d com.apple.quarantine` 移除隔离属性（旧版）→ 历史 SIP 绕过（Shrootless/MacDirtyCow CVE-2022-46689）→ 授权滥用（`disable-library-validation` 加载任意 dylib、`get-task-allow` 调试器注入）
- **升级**: 绕过 Gatekeeper 执行任意应用 / 绕过 SIP 写系统分区

### DYLD 注入 / Dylib 劫持
- **信号**: 目标 App 未启用 Hardened Runtime 或有 `allow-dyld-environment-variables` 授权
- **验证**: `DYLD_INSERT_LIBRARIES=./inject.dylib /path/to/target` + `__attribute__((constructor))` 构造函数注入 → 扫描可注入目标 `find /Applications -name "*.app"` + `codesign -d --entitlements` grep 授权 → Dylib 劫持三型（weak `LC_LOAD_WEAK_DYLIB` / `@rpath` / Proxy 重新导出原符号保功能）
- **升级**: 注入目标进程 → 继承其 TCC 权限 → 数据窃取
- **注意**: Hardened Runtime / SIP 保护二进制 / SUID / App Sandbox 均阻止 DYLD 注入
