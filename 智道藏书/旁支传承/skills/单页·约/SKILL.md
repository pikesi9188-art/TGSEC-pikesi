...
当目标 SPA（umi/webpack5 或 SvelteKit 打包）用前端加密调 API、用 js-websocket 推送、且需要精确参数但抓包不全时使用。2026-08 实测两条链路：AES-ECB API + js-websocket + PG SOFT（8G/kk8 TMA），以及 SvelteKit + 国密 SM2/SM4 clientapi（lj5888 蓝箭体育，详见 references/lj5888-lanjian-abpay-session.md）。
...
- 工作客户端模板:`scripts/sm_crypto_client.js`(SM4 认证 + SM2 请求/解密 + session 覆盖)
...
## 参考文件
- `references/kk8-8g-game-chain.md` — 8G/kk8 平台实测细节：game/enter 真参数、注册/登录格式、PG SOFT 链路（ot token/verify 1402/GetLaunchURLHTML）、WS 模块 ID 速查
- `references/lj5888-lanjian-abpay-session.md` — 蓝箭体育实测：SM2/SM4 完整协议、367 条 API 地图、订单状态机、ABpay 钱包绑定链、设备头链
- `scripts/sm_crypto_client.js` — 国密 SM2/SM4 clientapi 工作客户端模板（SM4 认证 + SM2 加解密 + 注册演示）
