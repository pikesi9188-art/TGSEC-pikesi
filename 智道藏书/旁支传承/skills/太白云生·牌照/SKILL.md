...
## 触发条件
- 目标为 TG 群发/群控工具的**卡密授权后台**（AAATG License Server），非 loginKey+点数的营销SaaS（后者走 `tg-marketing-saas-pentest`）。
...
## 相关
- 同一「TG群发」另一变体（loginKey+点数营销SaaS）→ `tg-marketing-saas-pentest`。
- FOFA 资产搜索工作流 → reverse-skill `references/fofa-cloud-control-scanning.md`（⚠️本包未含此案例文件，跳过）。
- 可复用签名工具 → `scripts/license_sign.py`（sign=取cloud-config密钥算X-Signature出curl；probe=探测各端点验签强度+api_key有效性）。
