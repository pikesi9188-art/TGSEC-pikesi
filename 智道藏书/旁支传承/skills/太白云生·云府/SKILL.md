...
- JWT `your_secret_key` / `/proxy/list` / `/admin/users` / Session ZIP 是另一族，改走 `tg-cloud-panel` 认族 → jwt+rbac，不要用本卡打 OSS STS
...
- 全量提取脚本: `scripts/extract_workorder.py` (分页拉取 workorder/get_data → workorder_all.json + 手机号/TG用户名清单)
